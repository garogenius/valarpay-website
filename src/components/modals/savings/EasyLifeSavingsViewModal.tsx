...
interface EasyLifeSavingsViewModalProps {
  isOpen: boolean;
  onClose: () => void;
  planId: string | null;
}

const EasyLifeSavingsViewModal: React.FC<EasyLifeSavingsViewModalProps> = ({ isOpen, onClose, planId }) => {
  const [openDeposit, setOpenDeposit] = React.useState(false);
  const [openWithdraw, setOpenWithdraw] = React.useState(false);
  const [openBreak, setOpenBreak] = React.useState(false);

  // Fetch plan details
  const { plan: easyLifePlanData, isPending } = useGetEasyLifePlanById(planId);

  if (!isOpen || !planId) return null;

  // Normalize EasyLife plan data to handle API field name variations
  const actualPlan: EasyLifePlan | null = easyLifePlanData ? {
    ...easyLifePlanData,
    name: easyLifePlanData.name || (easyLifePlanData as any).title,
    goalAmount: easyLifePlanData.goalAmount || (easyLifePlanData as any).targetAmount,
    interestRate: easyLifePlanData.interestRate ?? (easyLifePlanData as any).interestRatePerAnnum,
    penaltyRate: easyLifePlanData.penaltyRate ?? (easyLifePlanData as any).earlyWithdrawalPenaltyRate,
    earlyWithdrawalEnabled: easyLifePlanData.earlyWithdrawalEnabled ?? (easyLifePlanData as any).allowEarlyWithdrawal,
    totalDeposited: easyLifePlanData.totalDeposited ?? (easyLifePlanData as any).totalSaved ?? 0,
  } as EasyLifePlan : null;
  ...
  // EasyLife uses 'fundings' array, while also supports 'deposits'
  const transactions = ((actualPlan as any)?.fundings || (actualPlan as any)?.deposits || []) as Array<{
    id: string;
    amount: number;
    createdAt: string;
  }>;
  ...
  const earlyWithdrawalEnabled = actualPlan
    ? !!(actualPlan.earlyWithdrawalEnabled ?? (actualPlan as any).allowEarlyWithdrawal ?? false)
    : false;
  const canBreak = isEarly && earlyWithdrawalEnabled;
  const canWithdraw = !isLocked;

  const goalAmount = actualPlan?.goalAmount ?? (actualPlan as any)?.targetAmount ?? 0;
  const totalDeposited = actualPlan?.totalDeposited ?? (actualPlan as any)?.totalSaved ?? 0;
  const progress = goalAmount > 0 ? Math.min(100, (totalDeposited / goalAmount) * 100) : 0;
  const daysRemaining = (actualPlan as any)?.daysRemaining ?? (actualPlan?.maturityDate 
    ? (() => {
  ...
  return (
    <div className="z-[999999] overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 flex justify-center items-center w-full md:inset-0 h-[100dvh]">
      ...
      <div className="relative mx-2.5 2xs:mx-4 bg-bg-600 dark:bg-bg-1100 border border-border-800 dark:border-border-700 px-0 py-4 w-full max-w-md max-h-[92vh] rounded-2xl flex flex-col overflow-hidden">
        ...
        <div className="px-4 pt-1 pb-2">
          <h2 className="text-white text-xs font-medium">
            {actualPlan.name || (actualPlan as any).title || "EasyLife Savings Plan"}
          </h2>
        ...
        <div className="px-4 pb-3 flex-1 overflow-y-auto">
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className={`text-[10px] px-2 py-0.5 rounded-full ${
              actualPlan.status === "ACTIVE" 
                ...
                : actualPlan.status === "COMPLETED" || actualPlan.status === "CLOSED"
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div className="flex flex-col gap-0.5">
              ...
              <span className="text-white text-xs font-medium">₦{totalDeposited.toLocaleString()}</span>
            </div>
            <div className="flex flex-col gap-0.5 text-right">
              <span className="text-white/50 text-[10px]">Interest Earned</span>
              <span className="text-emerald-400 text-xs font-medium">+₦{(actualPlan.totalInterestAccrued ?? 0).toLocaleString()}</span>
            </div>
          </div>
          ...
          {goalAmount > 0 && (
            <div className="mb-3 p-2.5 bg-white/5 rounded-lg border border-white/10">
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span className="text-white text-[11px] font-medium">₦{goalAmount.toLocaleString()}</span>
              </div>
              ...
              <div className="flex items-center justify-between text-[9px] text-white/40">
                <span>₦{totalDeposited.toLocaleString()} saved</span>
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-2.5 pb-3 mb-3 border-t border-b border-white/10 pt-3">
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <span className="text-white text-[11px]">
                {actualPlan.durationDays ? `${actualPlan.durationDays} Days` : "N/A"}
              </span>
            </div>
            <div className="flex flex-col gap-0.5">
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <span className="text-white text-[11px]">
                {actualPlan.penaltyRate !== undefined && actualPlan.penaltyRate !== null
                  ? `${(actualPlan.penaltyRate * 100).toFixed(1)}%`
                  : "N/A"}
              </span>
            </div>
            {actualPlan.contributionFrequency && (
              <div className="flex flex-col gap-0.5">
                <span className="text-white/50 text-[10px]">Contribution Frequency</span>
                <span className="text-white text-[11px]">
                  {actualPlan.contributionFrequency}
                </span>
              </div>
      ...
      <SavingsDepositModal
        ...
        planName={actualPlan.name || (actualPlan as any).title || "EasyLife Savings Plan"}
...
export default EasyLifeSavingsViewModal;
